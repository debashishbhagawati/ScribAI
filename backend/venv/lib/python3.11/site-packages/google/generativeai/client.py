...
class _ClientManager:
    ...
    def configure(
        ...
        *,
        ...
        `GOOGLE_API_KEY` environment variable is set, it will be used as the API key. If not,
        ...
        Args:
            ...
            api_key: The API-Key to use when creating the default clients (each service uses
                ...
                If omitted, and the `GOOGLE_API_KEY` or the `GOOGLE_API_KEY` environment variable
        ...
        if had_api_key_value:
            ...
            if not api_key:
                # If the GOOGLE_API_KEY doesn't exist, attempt to load the
